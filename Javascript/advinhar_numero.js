let numUsuario = document.querySelector('#num-chute'),
    ...
    fundo = document.querySelector('#container')
...
tentarBtn.addEventListener('click', function(e){
    ...
    if(parseInt(numUsuario.value) === numSorteado) {
        ...
        window.location.reload()
    } else if (Math.abs(numSorteado - numUsuario.value) <= 2) {
    ...
    } else if (Math.abs(numSorteado - numUsuario.value) >= 3 && Math.abs(numSorteado - numUsuario.value) <= 4) {
        numChances <= 1 ? gameOver() : numChances -= 1
